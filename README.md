...
別途、Windows PC用の「RICOH抽選応募準備ツール」を用意しています。このPC版は、受付中の全商品を自動発見し、RICOH公式の表示価格と買取ルデヤの新品・完品買取価格を比較します。RICOH購入価格の方が高い商品はフォーム入力せず、価格条件を通過した商品だけを入力済み・送信直前のタブにします。最終送信は必ず本人が行います。
...
- 判定条件は、指定どおり `RICOH購入価格 > 買取ルデヤ価格` なら入力しない、同額なら入力対象です。
