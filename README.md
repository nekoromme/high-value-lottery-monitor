...
RICOH公式オンラインストアのGRシリーズ抽選販売を、GitHub Actionsで1日4回（日本時間3:17・9:17・15:17・21:17）監視するプログラムです。
...
初回結果に `"mode": "baseline(auto)"` と出れば正常です。この回は誤通知防止のためDiscord通知もCalendar登録もしません。以後は日本時間の3:17・9:17・15:17・21:17ごろに自動実行されます。

GitHub Actionsの予約実行は、混雑時に数時間遅れたり、まれに起動自体が抜けたりします。
その対策として `RICOH monitor automatic recovery` が毎時47分に本体の履歴だけを
確認します。最後の正常終了から7時間30分を超え、現在実行中でもない場合だけ本体を
自動で再起動します。復旧確認そのものはRICOHサイトへアクセスしないため、本体の監視回数を
むやみに増やさず、GitHub側の大幅遅延や予約欠落だけを補えます。
